...
class Evaluator:
    ...
    def _coco_predictions(self):
        ...
        for img_idx, img_dets in zip(self.img_indices, self.predictions):
            ...
            for det in img_dets:
                score = float(det[4])
                if score < .08:  # stop when below this threshold, scores in descending order
                    break
                image_id=str(img_id)
                if temp!=image_id:
                    temp=image_id
                    if j ==0:
                        coco_predictions +='"'+image_id+'.jpg":['
                        j+=1
                    else:
                        coco_predictions+=',"'+image_id+'.jpg":['
                bbox = "["+str(int(det[0]))+","+str(int(det[1]))+","+str(int(det[2]))+","+str(int(det[3]))+","
                bbox +=str('%.5f'%score)+"]"
                if i==0:
                    coco_predictions +=bbox
                    i = i+1
                else:
                    coco_predictions+=","+bbox
                # coco_det = dict(
